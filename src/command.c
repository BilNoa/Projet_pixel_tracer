...
/**
 * @brief Table des messages associés aux codes d'erreur.
 *
 * Index utilisés dans read_exec_command() :
 *   1 = commande inconnue
 *   2 = commande manquante (ligne vide)
 *   3 = erreur de paramètres
 *   4 = exit
 *   5 = clear
 *   6 = plot (redessine)
 *   7 = help
 *   8 = done (succès sans redessin)
 *   9 = id inconnu
 */
...
/** @brief Met la chaîne en minuscules (remplacement de strlwr non-portable). */
...
/**
 * @brief Passe en minuscules et tronque la chaîne au premier '#' ou '\n'.
 */
...
void read_from_stdin(Command * cmd) {
    ...
    /* Classe chaque token : mot → str_params, entier → int_params */
...
/**
 * @brief Vérifie que cmd contient exactement nb_str mots, nb_int entiers et nb_flt flottants.
 */
...
/**
 * @brief Vérifie les paramètres spécifiques à la commande polygon.
 *
 * Attend 1 mot (le nom), un nombre pair d'entiers ≥ 2 (les coordonnées x,y)
 * et aucun flottant.
 */
...
/**
 * @brief Lit une commande sur stdin, l'exécute et retourne un code d'erreur.
 *
 * Toutes les commandes sont traitées ici via une chaîne de if/else strcmp.
 * Les shapes créés sont ajoutés au current_layer de l'app.
 * Un goto end centralise l'affichage du message et la libération de cmd.
 */
int read_exec_command(Pixel_tracer_app * app) {
    ...
    if (strcmp(cmd_name, "exit") == 0) {
        if (!check_nb_params(cmd, 1, 0, 0)) { error_num = 3; goto end; }
        error_num = 4;
    }
        ...
        if (!check_nb_params(cmd, 1, 2, 0)) { error_num = 3; goto end; }
        Shape *sh = create_point_shape(cmd->int_params[0], cmd->int_params[1]);
        add_shape_to_layer(app->current_layer, sh);
        error_num = 0;
    }

    else if (strcmp(cmd_name, "line") == 0) {
        if (!check_nb_params(cmd, 1, 4, 0)) { error_num = 3; goto end; }
        Shape *sh = create_line_shape(cmd->int_params[0], cmd->int_params[1],
        ...
        if (!check_nb_params(cmd, 1, 3, 0)) { error_num = 3; goto end; }
        Shape *sh = create_square_shape(cmd->int_params[0], cmd->int_params[1],
                                        cmd->int_params[2]);
        add_shape_to_layer(app->current_layer, sh);
        error_num = 0;
    }
        ...
        if (!check_nb_params(cmd, 1, 4, 0)) { error_num = 3; goto end; }
        Shape *sh = create_rectangle_shape(cmd->int_params[0], cmd->int_params[1],
                                           cmd->int_params[2], cmd->int_params[3]);
        add_shape_to_layer(app->current_layer, sh);
        error_num = 0;
    }
        ...
        if (!check_nb_params(cmd, 1, 3, 0)) { error_num = 3; goto end; }
        Shape *sh = create_cercle_shape(cmd->int_params[0], cmd->int_params[1],
                                        cmd->int_params[2]);
        add_shape_to_layer(app->current_layer, sh);
        error_num = 0;
    }
        ...
        if (!check_nb_params_polygon(cmd)) { error_num = 3; goto end; }
        ...
        if (!check_nb_params(cmd, 1, 8, 0)) { error_num = 3; goto end; }
        Shape *sh =
    ...
    /* LIST : affiche areas, layers ou shapes de la sélection courante */
    else if (strcmp(cmd_name, "list") == 0) {
        if (!check_nb_params(cmd, 2, 0, 0)) { error_num = 3; goto end; }
        ...
        if (strcmp(cmd->str_params[1], "areas") == 0) {
            ...
            while (layer_node != NULL) {
                ...
                printf(layer == app->current_layer ? " * " : " - ");
                char vis = (layer->visible == 0) ? 'H' : 'V';
            ...
            char *tab_shape[] = { "POINT", "LINE", "SQUAR", "RECTANGLE", "CIRCLE", "POLYGON", "CURVE" };
            while (shape_node != NULL) {
                Shape *shp = (Shape *) shape_node->data;
                printf(shp == app->current_shape ? " * " : " - ");
                char info[200];
    ...
    /* NEW : crée une area (+ layer par défaut) ou un layer */
    ...
    /* SELECT : change current_area, current_layer ou current_shape par id */
    else if (strcmp(cmd_name, "select") == 0) {
        if (!check_nb_params(cmd, 2, 1, 0)) { error_num = 3; goto end; }
        ...
        if (strcmp(cmd->str_params[1], "area") == 0) {
            ...
            while (area_node != NULL) {
                ...
                if (area->id == cmd->int_params[0]) {
                    app->current_area = area;
                    lnode *layer_node = get_last_node(area->lst_layers);
                    app->current_layer = (Layer *) layer_node->data;
                    app->current_shape = NULL;
        ...
        if (strcmp(cmd->str_params[1], "layer") == 0) {
            lis make pixelst *layer_list = app->current_area->lst_layers;
            lnode *layer_node = get_first_node(layer_list);
            while (layer_node != NULL) {
                Layer *layer = (Layer *) layer_node->data;
    ...
    /* DELETE : supprime une shape du current_layer par id */
    ...
    /* SET : modifie les caractères d'affichage ou la visibilité d'un layer */
    else if (strcmp(cmd_name, "set") == 0) {
        if (!check_nb_params(cmd, 3, 1, 0)) { error_num = 3; goto end; }
